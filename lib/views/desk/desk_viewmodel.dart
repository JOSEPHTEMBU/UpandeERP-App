...
class DeskViewModel extends BaseViewModel {
  ...
  // late DesktopPageResponse desktopPage = DesktopPageResponse(); // Initialize with a default value

  late DesktopPageResponse desktopPage = DesktopPageResponse(
      message: DesktopPageMessage(
          charts: DesktopPageCharts(items: []),
          shortcuts: DesktopPageShortcuts(label: "", items: []),
          cards: DesktopPageCards(label: "label", items: []),
          allowCustomization: false));

  ErrorResponse? error;

  // DeskViewModel() {
  //   // Initialize currentModuleTitle with a default value
  //   currentModuleTitle = "";
  // }
  ...
  getDesktopPage() async {
    ...
    if (currentModule == null) {
      // Handle the case where currentModule is null.
      // You can return or set an error state as needed.
    } else {
      currentModuleTitle = "";
      Logger().d("deskviewmodel: currentmodule::$currentModule");
      try {
        _desktopPage = await locator<Api>().getDesktopPage(currentModule);
      } catch (e) {
        Logger().e("deskviewmodel: error $e");
        // Handle the error here, set an error state, or throw an exception.
      }
    }

    desktopPage = _desktopPage ?? desktopPage;
